...
export class dbUserController {
	...
	@UseInterceptors(FileInterceptor('avatar', {
		// diskStorage : sauvegarde le fichier dans ./uploads/ avec un nom unique par user
		storage: diskStorage({
			destination: './uploads',
	...
	async uploadAvatar(@UploadedFile() file: any, @Req() req) {
		...
		// supprime l'ancien avatar si présent avant d'en sauvegarder un nouveau
		const user = await this.dbUserService.getMyUser(userId);
		if (user?.avatarPath) {
			try { await unlink(join('/app', user.avatarPath)); } catch (_) {}
		}
