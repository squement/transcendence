...
export function update(ball: Ball, leftPaddle: Paddle, rightPaddle: Paddle, gameState: GameState, score: Score, keys: Keys, deltaTime: number, gameMode: string) {
	...
	else if ((ball.x >= (CANVAS_WIDTH - (PADDLE_WIDTH + PADDLE_SPACING)) && ball.x <= (CANVAS_WIDTH - (PADDLE_WIDTH + PADDLE_SPACING)) +  (Math.abs(ball.vx) * ball.speed)) && (ball.y > rightPaddle.y && ball.y < (rightPaddle.y + PADDLE_HEIGHT)) || (gameMode == "solo_training" && (ball.x >= (CANVAS_WIDTH - (PADDLE_WIDTH + PADDLE_SPACING))))) {ball.x = (CANVAS_WIDTH - (PADDLE_WIDTH + PADDLE_SPACING)); ball.vx = -ball.vx; if (ball.speed < MAX_BALL_SPEED) {ball.speed += BALL_ACCELERATION;} ball.vy += (Math.random() > 0.5 ? Math.random() / 3 : -(Math.random() / 3));}
