import { PADDLE_SPEED, PADDLE_HEIGHT, PADDLE_WIDTH, PADDLE_SPACING, MAX_BALL_SPEED, CANVAS_HEIGHT, CANVAS_WIDTH, BALL_ACCELERATION, SCORE_TO_WIN, BALL_SHIFTING, BALL_SPEED } from './game_config.js'
...
export function update(ball, leftPaddle, rightPaddle, gameOver, score, setShowGif, keys) {
	...
	if (ball.current.vy > 0 && (ball.current.x <= (PADDLE_WIDTH + PADDLE_SPACING) && ball.current.x > PADDLE_SPACING) && ((ball.current.y > leftPaddle.current.y) && (ball.current.y < (leftPaddle.current.y + Math.abs(ball.current.vy))))) {ball.current.y = leftPaddle.current.y; ball.current.vy = -ball.current.vy;}
	if (ball.current.vy < 0 && (ball.current.x <= (PADDLE_WIDTH + PADDLE_SPACING) && ball.current.x > PADDLE_SPACING) && ((ball.current.y < (leftPaddle.current.y + PADDLE_HEIGHT)) && (ball.current.y > (leftPaddle.current.y + PADDLE_HEIGHT - 10)))) {ball.current.y = (leftPaddle.current.y + PADDLE_HEIGHT); ball.current.vy = -ball.current.vy;}
	// Annoying ass collisions test right
	if (ball.current.vy > 0 && (ball.current.x >= (CANVAS_WIDTH - (PADDLE_WIDTH + PADDLE_SPACING)) && ball.current.x < (CANVAS_WIDTH - PADDLE_SPACING)) && ((ball.current.y > rightPaddle.current.y) && (ball.current.y < (rightPaddle.current.y + Math.abs(ball.current.vy))))) {ball.current.y = rightPaddle.current.y; ball.current.vy = -ball.current.vy;}
	if (ball.current.vy < 0 && (ball.current.x >= (CANVAS_WIDTH - (PADDLE_WIDTH + PADDLE_SPACING)) && ball.current.x < (CANVAS_WIDTH - PADDLE_SPACING)) && ((ball.current.y < (rightPaddle.current.y + PADDLE_HEIGHT)) && (ball.current.y > (rightPaddle.current.y + PADDLE_HEIGHT - 10)))) {ball.current.y = (rightPaddle.current.y + PADDLE_HEIGHT); ball.current.vy = -ball.current.vy;}
